...
function calculateBreakGlassRiskScore(session: BreakGlassSession): BreakGlassRiskScore {
  ...
  const riskLevel = totalScore >= 60 ? 'critical' : totalScore >= 40 ? 'high' :
...
async function breakGlassMiddleware(
...
async function activateBreakGlass(
  ...
  const credentialValid = await credentialService.verify(form.userId, role.metadata.credentialRequired!);
  if (!credentialValid) {
    reply.status(403).send({
...
interface BreakGlassReviewAuditEvent {
  ...
  verdict: 'adequate' | 'needs_investigation' | 'abuse_suspected' |
           'confirmed_adequate' | 'confirmed_abuse' | 'non_malicious_misuse';
