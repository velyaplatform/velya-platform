...
*Invalidated assumptions are documented separately in `docs/risk/invalidated-assumptions.md`. When a new technical decision is made that rests on an assumption not listed here, a new assumption entry must be created before the decision is finalized.*
