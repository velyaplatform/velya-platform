...
| AI-008 | The 8 web-tier cron agents (frota multi-agente em `apps/web/src/lib/agent-runtime.ts`) operating in shadow mode produce findings that match what a senior platform engineer would flag, with < 10% false positive rate           | Platform | High          | Compare 30 days of shadow findings against engineer triage decisions; track FP rate per office                                    | Assumed |
| AI-009 | Auto-quarantine via scorecard thresholds (validation < 0.6, evidence < 0.7, recurrence > 0.4) catches drifting agents before they take any safe-class action that confuses operators                                             | Platform | Medium        | Inject synthetic drift into one agent in dev; verify watchdog quarantines within 5 runs                                           | Assumed |
| AI-010 | The intent classifier in `lib/ai-agent-orchestrator.ts` (rule-based, no LLM) correctly routes ≥ 80% of natural-language operator queries to the right tool, with the semantic-search fallback covering the remaining 20%         | AI Team  | Medium        | Run 100 scripted operator queries; measure classification accuracy + fallback coverage                                            | Assumed |
| AI-011 | The cross-office validator structure in `agent-runtime.ts` (every agent validated by an agent in a different office) is a sufficient governance check until red team adversarial tests are added per `.claude/rules/red-team.md` | Platform | High          | Red team office reviews every promotion proposal before any agent leaves shadow stage                                             | Assumed |
