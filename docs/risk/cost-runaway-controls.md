...
async function enforceTokenBudget(agentId: string, estimatedTokens: number): Promise<void> {
  ...
  if (usage + estimatedTokens > budget.dailyTokenBudget) {
    ...
    await alerting.fire('agent_budget_exceeded', {
      agentId,
      budget: budget.dailyTokenBudget,
      usage,
    });

    if (usage + estimatedTokens > budget.dailyTokenBudget * 1.1) {
      throw new BudgetExceededError(
        `Agent ${agentId} daily budget exceeded; clinical review required to override`,
      );
    }
...
_Cost controls are reviewed monthly by the Platform Team lead. Any circuit breaker that fires must be reviewed within 24 hours to determine root cause. If a circuit breaker fires for a clinical AI service, the Clinical Medical Officer must be notified within 1 hour._
