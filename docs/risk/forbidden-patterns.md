...
{userRole === 'physician' && (
  <button onClick={approveDischarge}>Approve Discharge</button>
)}
...
*When a new forbidden pattern is identified from an incident, security review, or clinical safety concern, it must be added to this registry within one sprint cycle. Every engineer onboarding to Velya must complete a review of this document as part of their onboarding checklist.*
