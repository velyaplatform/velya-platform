...
export class AgentSprite {
  ...
  private taskBubbleBg?: Phaser.GameObjects.Graphics;
  private taskBubbleText?: Phaser.GameObjects.Text;
  private taskBubbleTail?: Phaser.GameObjects.Graphics;
  ...
  constructor(
    ...
    this.hitArea.on("pointerdown", () => {
      this.scene.events.emit("agentSelected", this.agent.id);
    });
  ...
  /**
   * Desenha um balão de fala acima do crachá com a tarefa atual.
   * Posição: acima do badge (labelY = y - 140, badge ~40px alto → bubble começa em y - 195).
   */
  showTaskBubble(task: string): void {
    if (!task) return;
    const truncated = task.length > 80 ? task.slice(0, 77) + '…' : task;
    const x = this.nameText.x;
    const labelY = this.nameText.y; // já é y - 140 + 5 = y - 135
    const bubbleY = labelY - 54;    // bem acima do crachá

    this.taskBubbleText = this.scene.add.text(x, bubbleY + 8, truncated, {
      fontFamily: '"Segoe UI", "Helvetica Neue", Arial, sans-serif',
      fontSize: '11px',
      fontStyle: 'normal',
      color: '#ffffff',
    ...
    this.taskBubbleText.setDepth(910);

    const textW = Math.max(this.taskBubbleText.width, 120);
    const textH = this.taskBubbleText.height;
    const bgW = textW + 20;
    const bgH = textH + 14;
    const bgX = x - bgW / 2;
    const bgY = bubbleY;

    this.taskBubbleBg = this.scene.add.graphics();
    this.taskBubbleBg.fillStyle(0x0a7d3a, 0.95); // verde escuro (trabalhando)
    this.taskBubbleBg.fillRoundedRect(bgX, bgY, bgW, bgH, 8);
    this.taskBubbleBg.lineStyle(1.5, 0x22c55e, 0.9);
    this.taskBubbleBg.strokeRoundedRect(bgX, bgY, bgW, bgH, 8);
    this.taskBubbleBg.setDepth(909);

    // Rabiguinho apontando pra baixo (pro crachá)
    this.taskBubbleTail = this.scene.add.graphics();
    this.taskBubbleTail.fillStyle(0x0a7d3a, 0.95);
    this.taskBubbleTail.beginPath();
    this.taskBubbleTail.moveTo(x - 6, bgY + bgH);
    this.taskBubbleTail.lineTo(x + 6, bgY + bgH);
    this.taskBubbleTail.lineTo(x, bgY + bgH + 8);
    this.taskBubbleTail.closePath();
    this.taskBubbleTail.fillPath();
    this.taskBubbleTail.setDepth(909);
  }
  ...
  destroy(): void {
    ...
    this.taskBubbleText?.destroy();
    this.taskBubbleTail?.destroy();
  }
}
