  ...
  return (
    <AppShell pageTitle={delegation.title}>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <span className="font-mono text-xs text-slate-400">{delegation.id}</span>
              <span
            ...
            <p className="page-subtitle">
              De <strong className="text-slate-200">{delegation.createdByName}</strong> para{' '}
              <strong className="text-slate-200">{delegation.assignedToName}</strong>
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        >
          <h2 id="desc-heading" className="text-xs uppercase tracking-wider font-semibold text-slate-300 mb-3">
            Descrição
          </h2>
          <p className="text-sm text-slate-100 whitespace-pre-wrap mb-4">{delegation.description}</p>
          {delegation.acceptanceCriteria && (
            <>
              <h3 className="text-xs uppercase tracking-wider font-semibold text-slate-300 mt-4 mb-2">
        ...
        >
          <h2 id="meta-heading" className="text-xs uppercase tracking-wider font-semibold text-slate-300 mb-3">
            Metadados
          </h2>
          <dl className="text-sm space-y-2">
            <div className="flex justify-between gap-2">
              <dt className="text-slate-400">Categoria</dt>
              <dd className="text-slate-100">{delegation.category}</dd>
            </div>
            {delegation.dueAt && (
              <div className="flex justify-between gap-2">
            ...
            {delegation.location && (
              <div className="flex justify-between gap-2">
                ...
                <dd className="text-slate-100 text-right">{delegation.location}</dd>
              </div>
            )}
            ...
            <div className="flex justify-between gap-2">
              <dt className="text-slate-400">Atualizada</dt>
              <dd className="text-slate-100">
      ...
      {transitions.length > 0 && (
        ...
        >
          <h2 id="actions-heading" className="text-xs uppercase tracking-wider font-semibold text-slate-300 mb-3">
            Ações
          </h2>
          <div className="flex flex-col gap-3">
            <label htmlFor="status-note" className="text-sm font-medium text-slate-200">
              Comentário (opcional)
            </label>
            <textarea
              id="status-note"
              ...
              rows={2}
              className="bg-slate-800 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400"
        ...
        <ol className="border-l border-slate-700 ml-2 space-y-4">
          {delegation.history.map((entry, idx) => (
            <li key={idx} className="pl-4 relative">
              ...
              {entry.note && (
                <div className="text-sm text-slate-300 mt-1 italic">"{entry.note}"</div>
              )}
            </li>
          ))}
        </ol>
