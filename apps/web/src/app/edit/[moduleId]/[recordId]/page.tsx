  ...
  if (!module) {
    return (
      <AppShell pageTitle="Módulo desconhecido">
        <div role="alert" className="bg-neutral-50 border border-neutral-300 text-neutral-700 rounded-md px-4 py-3">
  ...
  return (
    <AppShell pageTitle={`Editar ${mod.title}`}>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="page-subtitle">
              Registro <span className="font-mono text-neutral-700">{recordId}</span>
              {record.hasOverride && (
                <span className="ml-2 text-xs px-2 py-0.5 rounded-full bg-neutral-100 text-neutral-700 border border-neutral-300">
                  Editado
                </span>
              )}
              {record.isNew && (
                <span className="ml-2 text-xs px-2 py-0.5 rounded-full bg-neutral-50 text-neutral-900 border border-neutral-300">
                  Novo
                </span>
              )}
            </p>
      ...
      {!canEdit && (
        <div
          role="alert"
          className="bg-neutral-50 border border-neutral-300 text-neutral-700 text-sm rounded-md px-4 py-3 mb-4"
        >
          Sua funcao tem permissao apenas de leitura para este módulo. Os campos abaixo são
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          {success && (
            <div role="status" className="mb-4 bg-neutral-50 border border-neutral-200 text-neutral-700 text-sm rounded-md px-4 py-3">
              {success}
            </div>
          )}
            ...
            {mod.columns.map((col) => {
                ...
                return (
                  <FieldRow key={col.key} label={col.label} fullWidth={false}>
                    <input
                      ...
                      readOnly
                      className="w-full min-h-[44px] bg-neutral-50/60 border border-neutral-200 rounded-md px-3 py-2 text-sm text-neutral-500 cursor-not-allowed font-mono"
                    />
                  </FieldRow>
                );
              }
              ...
              return (
                <FieldRow key={col.key} label={col.label} fullWidth={isFullWidth} required={col.required}>
                  ...
                  />
                  {col.help && <p className="text-xs text-neutral-500 mt-1">{col.help}</p>}
                </FieldRow>
              );
            })}
          </div>
          ...
          {canEdit && (
            <>
              <div className="mt-6 flex flex-col gap-1.5">
                <label htmlFor="change-note" className="text-sm font-medium text-neutral-700">
                ...
                <input
                  ...
                  className="w-full min-h-[44px] bg-neutral-50 border border-neutral-300 rounded-md px-3 py-2 text-sm text-neutral-900 placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-neutral-200"
                />
              </div>
              ...
              <div className="flex items-center justify-between gap-3 mt-6 flex-wrap">
                <button
                  ...
                  disabled={saving}
                  className="min-h-[44px] inline-flex items-center px-4 py-2 rounded-md bg-neutral-50 border border-neutral-300 text-neutral-700 hover:bg-neutral-100 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-neutral-200 disabled:opacity-60"
                >
                  Remover registro
                </button>
                <div className="flex gap-3 ml-auto">
                  ...
                  <button
                    ...
                    className="min-h-[44px] inline-flex items-center px-5 py-2 rounded-md bg-neutral-900 hover:bg-neutral-900 text-white text-sm font-bold focus:outline-none focus:ring-2 focus:ring-neutral-200 disabled:opacity-60 disabled:cursor-not-allowed"
                  >
        ...
        >
          ...
          {history.length === 0 ? (
            <p className="text-sm text-neutral-500">Nenhuma alteração registrada ainda.</p>
          ) : (
            <ol className="border-l border-neutral-200 ml-2 space-y-4">
              {[...history].reverse().map((entry, idx) => (
                <li key={idx} className="pl-4 relative">
                  ...
                  <div className="text-sm text-neutral-900 font-semibold mt-0.5">
                    {entry.actor}
                  </div>
                  {entry.note && (
                    <div className="text-xs text-neutral-500 italic mt-1">"{entry.note}"</div>
                  )}
                  {entry.fieldChanges.length > 0 && (
                    <ul className="mt-2 space-y-1">
                      {entry.fieldChanges.map((change, i) => (
                        <li key={i} className="text-xs text-neutral-500">
                          <span className="text-neutral-500 font-mono">{change.field}:</span>{' '}
                          <span className="text-neutral-500 line-through">
                            {formatValue(change.from)}
                          </span>{' '}
                          → <span className="text-neutral-900">{formatValue(change.to)}</span>
                        </li>
                      ))}
                    </ul>
                  )}
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${fullWidth ? 'sm:col-span-2' : ''}`}>
      <label className="text-sm font-medium text-neutral-700">
        {label}
        {required && (
          <span aria-hidden="true" className="text-neutral-500 ml-0.5">
            *
          </span>
        )}
      </label>
...
function FieldInput({
  ...
  const baseClass =
    'w-full min-h-[44px] bg-neutral-50 border border-neutral-300 rounded-md px-3 py-2 text-sm text-neutral-900 placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-neutral-200 disabled:opacity-60 disabled:cursor-not-allowed';
    ...
    case 'boolean':
      return (
        <label className="inline-flex items-center gap-2 min-h-[44px]">
          ...
          />
          <span className="text-sm text-neutral-700">{value ? 'Sim' : 'Não'}</span>
        </label>
      );
    case 'tags':
      return (
