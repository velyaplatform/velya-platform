...
export function RelatedItems({
  ...
  return (
    <section
      ...
      className={`bg-slate-900 border border-slate-700 rounded-xl p-5 ${className ?? ''}`}
    >
      <h2
        id={`related-${entityType}-${entityId}-heading`}
        className="text-xs uppercase tracking-wider font-semibold text-slate-300 mb-3 flex items-center gap-2"
      >
        <span aria-hidden="true">{'\uD83D\uDD17'}</span> {title}
      </h2>

      {loading && <p className="text-sm text-slate-400">Carregando relacionados...</p>}
      {error && (
            ...
            <li key={group.module}>
              <div className="flex items-center justify-between gap-2 mb-1">
                <h3 className="text-sm font-bold text-slate-100">
                  {group.label}{' '}
                  <span className="text-xs text-slate-400 font-normal">({group.count})</span>
                </h3>
                <Link
                  href={group.href}
                  className="text-xs text-blue-300 hover:text-blue-200 underline-offset-2 hover:underline focus:outline-none focus:ring-2 focus:ring-blue-300 rounded"
                >
                  Ver todos →
                </Link>
              </div>
              <ul className="flex flex-col gap-1">
                {group.sample.map((entry) => (
                  <li key={entry.id}>
                    <Link
                      href={entry.href}
                      className="block px-3 py-2 text-xs text-slate-200 bg-slate-800 border border-slate-700 rounded-md hover:bg-slate-700 hover:border-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-300"
                    >
                      <span className="font-mono text-blue-300 mr-2">{entry.id}</span>
