      ...
      {isOpen && (
        ...
        >
          {results.length === 0 && debounced.trim().length >= 2 && (
            <li className="px-3 py-2 text-sm text-slate-400">Nenhum medicamento encontrado</li>
          )}
          {results.map((opt, idx) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-sm text-slate-100 font-semibold">{opt.name}</span>
                  {opt.source === 'stock' && (
                    <span className="ml-auto text-[10px] px-2 py-0.5 rounded-full bg-green-900/40 text-green-200 border border-green-700/60">
                      em estoque
                    </span>
                  )}
                </div>
