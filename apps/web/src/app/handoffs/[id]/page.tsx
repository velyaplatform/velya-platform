...
const SEVERITY_BADGE: Record<IllnessSeverity, string> = {
  stable: 'bg-green-900/40 text-green-200 border-green-700/60',
  watcher: 'bg-amber-900/40 text-amber-200 border-amber-700/60',
  unstable: 'bg-red-900/40 text-red-200 border-red-700/60',
};
  ...
  return (
    <AppShell pageTitle={`Plantão ${handoff.ward}`}>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <span className="font-mono text-xs text-slate-400">{handoff.id}</span>
              <span
              ...
              <span className="text-xs text-blue-300">{handoff.shiftLabel}</span>
            </div>
            <h1 className="page-title">{handoff.ward}</h1>
            <p className="page-subtitle">
              De <strong className="text-slate-100">{handoff.fromUserName}</strong> para{' '}
              <strong className="text-slate-100">{handoff.toUserName}</strong> ·{' '}
      ...
      <section className="bg-blue-950/30 border border-blue-700/60 rounded-xl p-5 mb-4">
        ...
        {aiError && (
          <div role="alert" className="text-xs text-red-200 mb-2">
            ⚠ {aiError}
          </div>
        )}
        {handoff.aiSummary ? (
          <p className="text-sm text-blue-100 whitespace-pre-wrap">{handoff.aiSummary}</p>
        ) : (
      ...
      {handoff.unitNotes && (
        ...
        >
          <h2 id="unit-notes-heading" className="text-xs uppercase tracking-wider font-bold text-amber-200 mb-2">
            Avisos da unidade
          </h2>
      ...
      <ul className="flex flex-col gap-3 mb-6">
        {handoff.patients.map((p, idx) => (
          ...
          >
            <header className="flex items-start justify-between gap-3 flex-wrap mb-3">
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-mono text-blue-300 text-sm">{p.patientMrn}</span>
                  <span className="text-base font-bold text-slate-100">{p.patientName}</span>
                  <span
            ...
            <div className="text-xs uppercase tracking-wider text-slate-400 font-semibold mt-3 mb-1">
              <strong>P</strong> — Patient summary
            </div>
            <p className="text-sm text-slate-100 whitespace-pre-wrap">{p.patientSummary}</p>

            {p.actionItems.length > 0 && (
              <>
                <div className="text-xs uppercase tracking-wider text-slate-400 font-semibold mt-4 mb-1">
                  ...
                  {p.actionItems.map((a, i) => (
                    <li key={i} className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        <span>{a.task}</span>
                        {a.owner && (
                          <span className="text-xs text-slate-400 ml-2">→ {a.owner}</span>
                        )}
                        {a.dueAt && (
            ...
            {p.situationAwareness && (
              <>
                ...
                <p className="text-sm text-slate-100 whitespace-pre-wrap">{p.situationAwareness}</p>
              </>
            )}
      ...
      {handoff.status !== 'completed' && (
        <section
          ...
          className="bg-slate-900 border border-slate-700 rounded-xl p-5"
        >
          <h2 id="readback-heading" className="text-sm font-bold text-slate-100 mb-3">
          ...
          <form onSubmit={handleReceive}>
            <textarea
              ...
              rows={4}
              className="w-full bg-slate-800 border border-slate-600 rounded-md px-3 py-2 text-sm text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400 min-h-[100px]"
      ...
      {handoff.receiverReadback && (
        <section className="bg-green-950/30 border border-green-700/60 rounded-xl p-5 mt-4">
          <h2 className="text-sm font-bold text-green-200 mb-2">
      ...
      <section
        ...
        className="bg-slate-900 border border-slate-700 rounded-xl p-5 mt-4"
      >
        <h2 id="history-heading" className="text-xs uppercase tracking-wider font-semibold text-slate-300 mb-3">
          Histórico auditado
        </h2>
        <ol className="border-l border-slate-700 ml-2 space-y-3">
          {handoff.history.map((entry, idx) => (
            <li key={idx} className="pl-4 relative">
              ...
              {entry.note && <div className="text-xs text-slate-300 italic">"{entry.note}"</div>}
            </li>
          ))}
        </ol>
      </section>
