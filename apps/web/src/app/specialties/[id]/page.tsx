  ...
  return (
    <AppShell pageTitle={specialty.name}>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <span className="font-mono text-xs text-neutral-500">CFM {specialty.cfmCode}</span>
              <span className="text-xs px-2 py-1 rounded-full bg-neutral-100 text-neutral-900 border border-neutral-300">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <Kpi label="Em plantao agora" value={onDutyCount} />
        <Kpi label="Pacientes atribuidos" value={totalAssignedPatients} />
        <Kpi label="Alas onde atua" value={relatedWards.length} />
      </div>
            ...
            {involvedStaff.map((s) => {
              ...
              return (
                <li key={s.id}>
                  <Link
                    href={`/employees/${s.id}`}
                    className="block bg-neutral-50 border border-neutral-200 rounded-lg p-4 hover:bg-neutral-100 hover:border-neutral-300 focus:outline-none focus:ring-2 focus:ring-neutral-200 transition-colors"
                  >
                    <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
                      <div className="min-w-0 flex-1">
                        <h3 className="text-sm font-bold text-neutral-900">{s.name}</h3>
                    ...
                    <div className="flex items-center justify-between text-xs text-neutral-500 mt-2">
                      ...
                      {s.contactExtension && (
                        <span className="font-mono text-neutral-700">ramal {s.contactExtension}</span>
                      )}
                    </div>
      ...
      >
        <h2
          ...
          className="text-base font-bold text-neutral-900 mb-3 flex items-center gap-2"
        >
          Alas onde atua ({relatedWards.length})
        </h2>
              ...
              <li key={w.id}>
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-1">
                    ...
                    <span className="text-[10px] text-neutral-500">{w.location.sector}</span>
                  </div>
                  <p className="text-xs text-neutral-500">
...
function ListSection({
  ...
  return (
    <section className="bg-white border border-neutral-200 rounded-xl p-5">
      <h3 className="text-xs uppercase tracking-wider font-semibold text-neutral-500 mb-3 flex items-center gap-2">
        <span aria-hidden="true">{icon}</span> {title} ({items.length})
      </h3>
      {items.length === 0 ? (
        <p className="text-sm text-neutral-500">Nenhum item cadastrado.</p>
      ) : (
        <ul className="text-sm text-neutral-900 space-y-1">
          {items.map((item, i) => (
            <li key={i} className="flex items-baseline gap-2">
              {item.secondary && (
                <span className="font-mono text-[11px] text-neutral-700 shrink-0">
                  {item.secondary}
                </span>
              )}
