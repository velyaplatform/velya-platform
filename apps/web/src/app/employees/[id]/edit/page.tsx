...
export default function EmployeeEditPage() {
  ...
  const original = STAFF.find((s) => s.id === params.id);

  const [form, setForm] = useState(() => ({
    name: original?.name ?? '',
    role: (original?.role ?? 'medico') as ProfessionalRole,
    specialty: original?.specialty ?? '',
    council: original?.council ?? '',
    ward: original?.ward ?? '',
    shift: (original?.shift ?? 'manha') as ShiftType,
    shiftStart: original?.shiftStart ?? '07:00',
    shiftEnd: original?.shiftEnd ?? '19:00',
    contactExtension: original?.contactExtension ?? '',
  }));
  ...
  if (!original) {
    return (
      <AppShell pageTitle="Editar funcionário — registro ausente">
        <div className="page-header">
          <h1 className="page-title">Editar funcionário — registro ausente</h1>
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    setMessage(null);
    // Placeholder: in production this posts to /api/employees/[id]
    await new Promise((r) => setTimeout(r, 500));
    setMessage('Alterações registradas (modo demonstração — API /api/employees ainda não implementada).');
    setSaving(false);
    setTimeout(() => router.push(`/employees/${params.id}`), 1500);
  }

  const input =
    'w-full min-h-[44px] bg-slate-50 border border-slate-300 rounded-md px-3 py-2 text-slate-900 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400';
  ...
  return (
    <AppShell pageTitle={`Editar ${original.name}`}>
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Matrícula <span className="font-mono text-slate-900">{original.id}</span> ·{' '}
          {ROLE_LABELS[original.role]}
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className={input}
              placeholder="Ex: Clínica Médica, UTI, Cirurgia Geral"
            />
          </div>
