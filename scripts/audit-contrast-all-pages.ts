...
async function findContrastIssues(p: Page, pageName: string): Promise<ContrastIssue[]> {
  return p.evaluate(function findContrastIssuesEval(pageName: string): ContrastIssue[] {
      ...
      function ratio(rgb1: [number, number, number], rgb2: [number, number, number]): number {
...
async function run() {
  ...
  for (const path of PAGES) {
    ...
    try {
      ...
      await p.screenshot({ path: OUT + '/' + path.replace(/[/\\[\\]]/g, '_') + '.png', fullPage: true });

      // axe
      const axe = await new AxeBuilder({ page: p }).withTags(['wcag2aa', 'wcag22aa']).analyze();
      for (const v of axe.violations) {
        if (v.impact === 'critical' || v.impact === 'serious') {
          axeViolations.push({ page: path, id: v.id, impact: v.impact || 'unknown', nodes: v.nodes.length });
  ...
  console.log('\\n══════════════════════════════════════════');
  console.log('  TOTAL: ' + allIssues.length + ' contrastes ruins, ' + axeViolations.length + ' axe critical/serious');
  console.log('══════════════════════════════════════════');
  ...
  if (totalContrastNodes > maxAllowed) {
    console.error(
      '\\n❌ COMPLIANCE FAIL: ' + totalContrastNodes + ' color-contrast nodes excedem o teto VELYA_MAX_CONTRAST_NODES=' + maxAllowed,
    );
    process.exitCode = 2;
  } else {
