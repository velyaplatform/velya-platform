...
SYNC_SCRIPT="${REPO_ROOT}/scripts/sync-agent-coordination.mjs"
OUT_FILE="${REPO_ROOT}/ops/state/agent-sync-status.json"

STATE_ROOT="${VELYA_AUTOPILOT_STATE_DIR:-${HOME}/.local/state/velya/autopilot}"
SERVICE_DIR="${STATE_ROOT}/services"
LOG_FILE="${SERVICE_DIR}/agent-coordination-sync.log"
PID_FILE="${SERVICE_DIR}/agent-coordination-sync.pid"
HEALTH_FILE="${SERVICE_DIR}/agent-coordination-sync.last.json"
STALE_AFTER_SECONDS="${VELYA_AGENT_COORDINATION_STALE_AFTER_SECONDS:-180}"

info() {
  printf '[agent-coordination-sync] %s\n' "$*"
}
...
for _ in $(seq 1 20); do
  if [[ -f "${OUT_FILE}" ]] && health_is_fresh; then
    info "sync ativo"
    info "pid: ${pid}"
    info "log: ${LOG_FILE}"
    exit 0
