...
SYNC_SCRIPT="${REPO_ROOT}/scripts/sync-lincesoc-platform-ops.mjs"

STATE_ROOT="${VELYA_AUTOPILOT_STATE_DIR:-${HOME}/.local/state/velya/autopilot}"
SERVICE_DIR="${STATE_ROOT}/services"
LOG_FILE="${SERVICE_DIR}/lincesoc-platform-ops-sync.log"
PID_FILE="${SERVICE_DIR}/lincesoc-platform-ops-sync.pid"
HEALTH_FILE="${SERVICE_DIR}/lincesoc-platform-ops-sync.last.json"
STATUS_FILE="${REPO_ROOT}/docs/orchestration/lincesoc-platform-ops-status-current.md"
STALE_AFTER_SECONDS="${VELYA_LINCESOC_PLATFORM_SYNC_STALE_AFTER_SECONDS:-180}"

info() {
  printf '[lincesoc-platform-sync] %s\n' "$*"
}
...
cd "${REPO_ROOT}"
setsid sh -c "exec node '${SYNC_SCRIPT}' --interval 45 --health-file '${HEALTH_FILE}' >'${LOG_FILE}' 2>&1" < /dev/null &
pid=$!
echo "${pid}" > "${PID_FILE}"
